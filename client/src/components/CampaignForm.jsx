...
import { crowdfundingContract } from '../utils/thirdweb-config';
...
const CampaignForm = () => {
	...
	const handleCreateFundraiserOnChain = async ({
		_name,
		_targetFunds,
		_deadline,
	}) => {
		...
		const transaction = await prepareContractCall({
			...
			method:
				'function createProject(string _name, uint256 _targetFunds, uint256 _deadline)',
			params: [_name, _targetFunds, _deadline],
		});
		const sentTrans = sendTransaction(transaction);
		console.log('Transaction hash:', sentTrans);
	};
	...
	const onSubmit = async (data) => {
		try {
			...
			await handleCreateFundraiserOnChain({
				...
				_deadline: Date.now(data.deadline),
			});
			const payload = {
				title: data.title,
